...
# pickup (aka pop) takes a given number of items item off the top of the
# specified stack and stores it in the 'crane' variable. If the crane is
# already in use or if the stack does not have that many items, this
...
pickup() {
    ...
    if (( ${#stacks[$stackid]} < $count )); then
        echo "ERROR: Operator Failure: There are not $count boxes available in stack $stackid!" >&2
        exit 1
    fi

    # the "top" of the stack is the front of the string
    crane=${stacks[$stackid]:0:${count}}
    stacks[$stackid]=${stacks[$stackid]:${count}}
}
