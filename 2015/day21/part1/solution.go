...
func main() {
	...
	var cheapestLoadout *Loadout
	for _, loadout := range loadouts {
		if loadout.CanWin() {
			if cheapestLoadout == nil || cheapestLoadout.Cost() > loadout.Cost() {
				cheapestLoadout = loadout
			}
			winningLoadouts = append(winningLoadouts, loadout)
		}
	}
	fmt.Printf("%d loadouts can win!\n", len(winningLoadouts))
	fmt.Printf("Cheapest loadout costs %d gold!\n", cheapestLoadout.Cost())
	fmt.Printf("  Weapon: %s\n", cheapestLoadout.Weapon.Name())
	fmt.Printf("   Armor: %s\n", cheapestLoadout.Armor.Name())
	fmt.Printf("  Ring 1: %s\n", cheapestLoadout.Ring1.Name())
	fmt.Printf("  Ring 2: %s\n", cheapestLoadout.Ring2.Name())
}
