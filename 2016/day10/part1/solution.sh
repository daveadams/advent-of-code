...
declare -A bots inputs outputs bot_inputs
...
# find the bot that processed values 17 and 61
for id in "${!bot_inputs[@]}"; do
    if [[ ${bot_inputs[$id]} == "17 61" ]]; then
        echo $id
        exit 0
    fi
done

echo "OOOPS: Failed to find the right bot"
exit 1
